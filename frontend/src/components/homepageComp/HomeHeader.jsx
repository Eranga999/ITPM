      ...
      <div className="container mx-auto px-4 py-3 md:py-4">
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <Link to="/home" className={`font-medium hover:text-blue-600 transition-colors duration-200 ${isActive('/home')}`}>
              Home
            </Link>
            <Link to="/customer-dashboard" className={`flex items-center space-x-2 font-medium hover:text-blue-600 transition-colors duration-200 ${isActive('/customer-dashboard')}`}>
              <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                <path fillRule="evenodd" d="M12 11a4 4 0 100-8 4 4 0 000 8zm-7 10a7 7 0 0114 0H5z" clipRule="evenodd" />
              </svg>
              <span>Customer</span>
            </Link>
            <Link to="/tracking" className={`font-medium hover:text-blue-600 transition-colors duration-200 ${isActive('/tracking')}`}>
              Track
            </Link>
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl py-2">
                  {notifications.length > 0 ? (
            ...
            <button 
              onClick={handlebook} 
              className="bg-blue-600 text-white px-4 py-2 rounded-full hover:bg-blue-700 transition-colors duration-200 shadow-md"
            >
              Book Now
            </button>
          </nav>
        ...
        {isMenuOpen && (
          <nav className="mt-4 md:hidden pb-4 space-y-3">
            <Link to="/home" className={`block py-2 px-4 hover:bg-gray-100 rounded-lg ${isActive('/home')}`}>
              Home
            </Link>
            <Link to="/customer-dashboard" className={`block py-2 px-4 hover:bg-gray-100 rounded-lg ${isActive('/customer-dashboard')}`}>
              Account
            </Link>
            <Link to="/support" className={`block py-2 px-4 hover:bg-gray-100 rounded-lg ${isActive('/support')}`}>
              Support
            </Link>
            <button 
              onClick={handlebook}
